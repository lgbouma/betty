...
def make_posterior_table(pklpath, priordict, outpath, modelid, makepdf=1,
    ...
    PARAMFMTDICT = {
        ...
        'b':('({:.3f}; {:.3f})', 3, '--', '$b^{(2)}$') if parametrization=='log_depth_and_b' else (None, 3, '--', '$b$'),
        'u_star[0]':(None, 2, '--', "$u_1$"),
        'u_star[1]':(None, 2, '--', "$u_2$"),
        'u[0]':('({:.3f}; {:.3f})', 2, '--', "$u_1$"),
        'u[1]':('({:.3f}; {:.3f})', 2, '--', "$u_2$"),
        ...
        'f':('({:.3f}; {:.3f})', 3, '--', r"$f$"),
        'log_f':('({:.3f}; {:.3f})', 3, '--', r"$\log f$"),
        'depth':('--', 4, '--', r"$\delta$"),
        'r':('--', 3, '--', r"$R_{\rm p}/R_\star$"),
        'ror':('--', 3, '--', r"$R_{\rm p}/R_\star$"),
        'rho_star':('--', 2, 'g$\ $cm$^{-3}$', r"$\rho_\star$"),
        'r_planet':('--', 3, '$R_{\mathrm{Jup}}$', r"$R_{\rm p}$"),
        'a_Rs':('--', 2, '--', "$a/R_\star$"),
        'cosi':('--', 3, '--', '$\cos i$'),
        'T_14':('--', 2, 'hr', '$T_{14}$'),
        'T_13':('--', 2, 'hr', '$T_{13}$'),
        'mean':('({:.3f}; {:.3f})', 3, '--', r"$\langle f \rangle$"),
        'tess_mean':('({:.3f}; {:.3f})', 3, '--', r"$\langle f \rangle$"),
        'kepler_mean':('({:.3f}; {:.3f})', 3, '--', r"$\langle f \rangle$"),
    }
    ...
    for _p in _params:
        df1[_p] = np.round((float(df1[_p])*u.Rjup).to(u.Rearth).value,2)
